...
4. **Creating Layouts**
    ...
    - Make sure your layout contains 4 files: `index.js`, `layout-name.component.jsx`, `layout-name.data.js`
      and `layout-name.styles.jsx`.
    ...
    - Add all the layouts with the corresponding variations and name in `pages/layouts.js` - this will serve as a
      directory later on for ourselves and the client, make sure to always keep it updated and working.
...
5. **Pulling data from WordPress**
    ...
    - In `gatsby-node.js` make sure the `createPages` is uncommented function to start generating pages from WP (use
      createPosts for blog posts and add more if needed)
